...
function getMediaEventPayload(event:VideoEvent):DOMEventParams{
    const {currentTime, duration, error, videoWidth, videoHeight} = 
        event.currentTarget as unknown as HTMLVideoElement
    return {
        currentTime,
        duration,
...
>((props, ref)=>{
    ...
    const handleOnLoadedMetadata = useCallback((event:VideoEvent)=>{
        const videoNode = event.currentTarget
        if(videoNode){
            const {videoWidth, videoHeight} = videoNode as unknown as HTMLVideoElement
